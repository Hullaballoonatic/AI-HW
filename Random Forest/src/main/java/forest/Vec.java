...
public class Vec
{
	...
	{
		double d = 0.0;
		for (double aVec : vec) d += aVec * aVec;
		return d;
	}
	...
	public static double[] mean(List<double[]> vecs) {
		double[] result = new double[vecs.size()];
		for (double[] element : vecs) {
			add(result, element);
		}
		scale(result, 1.0 / vecs.size());
		return result;
	}
}
