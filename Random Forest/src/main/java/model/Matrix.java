...
public class Matrix
{
	...
	public Matrix(Matrix that)
	{
		...
		setSize(that.getNumRows(), that.getNumCols());
		copyBlock(0, 0, that, 0, 0, that.getNumRows(), that.getNumCols()); // (copies the meta data too)
	}
	...
	public void loadARFF(String filename) {
		...
		try (Scanner s = new Scanner(file)) {
			...
			throw new IllegalArgumentException("Failed to open file: " + file.getAbsolutePath() + ".");
		}
	}
	...
	/// Makes a getNumRows-by-columns matrix of *ALL CONTINUOUS VALUES*.
	...
	public void setSize(int rows, int cols)
	{
		...
		// Make space for each of the columns, then each of the getNumRows
		newColumns(cols);
		newRows(rows);
	}
	...
	/// to give the matrix some getNumRows.
	@SuppressWarnings("unchecked")
	...
	/// a continuous attribute.) This method also sets the number of getNumRows to 0, so
	/// you will need to call newRow or newRows when you are done adding columns.
	public void newColumn(int vals)
	{
		m_data.clear();
		String name = "col_" + getNumCols();
	...
	public double[] newRow()
	{
		int c = getNumCols();
		if (c == 0)
			throw new IllegalArgumentException("You must add some columns before you add any getNumRows.");
	...
	/// Adds 'n' new getNumRows to this matrix
	...
	/// Returns the number of getNumRows in the matrix
	public int getNumRows() { return m_data.size(); }


	/// Returns the number of columns (or attributes) in the matrix
	public int getNumCols() { return m_attr_name.size(); }
	...
	/// Swaps the positions of the two specified getNumRows
	...
	{
		setSize(that.getNumRows(), that.getNumCols());
		copyBlock(0, 0, that, 0, 0, that.getNumRows(), that.getNumCols());
	}
	...
	{
		if (destRow + rowCount > this.getNumRows() || destCol + colCount > this.getNumCols())
			throw new IllegalArgumentException("Out of range for destination matrix.");
		if (rowBegin + rowCount > that.getNumRows() || colBegin + colCount > that.getNumCols())
		...
		for (int i = 0; i < rowCount; i++)
		{
			...
			if (colCount >= 0) System.arraycopy(source, colBegin, dest, destCol, colCount);
		}
	}
	...
	{
		if(that.getNumRows() != this.getNumRows() || that.getNumCols() != this.getNumCols())
	...
	{
		setAll(0.0);
		int m = Math.min(getNumCols(), getNumRows());
		for(int i = 0; i < m; i++)
			row(i)[i] = 1.0;
	}
	...
	private static class SortComparator implements Comparator<double[]>
	{
		...
		{
			if ((ascending && a[column] < b[column]) || (!ascending && a[column] > b[column]))
				return -1;
			else if ((ascending && a[column] > b[column]) || (!ascending && a[column] < b[column]))
				return 1;
			else return 0;
		}
	}
